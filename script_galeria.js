...
// Mostrar sección de subida solo si hay usuario (aunque no usamos auth)
        ...
        galleries.forEach(gallery => {
            ...
            card.innerHTML = `
                ...
                <div class="gallery-info">
                    ...
                    <h3 class="gallery-title"><i class="${gallery.icon || 'fas fa-heart'} gallery-icon"></i> ${gallery.title || 'Sin título'}</h3>
...
// Subir nueva galería (campos opcionales)
uploadForm.addEventListener('submit', async (e) => {
    e.preventDefault();

    const title = document.getElementById('galleryTitle').value || 'Sin título';
    const icon = document.getElementById('galleryIcon').value || 'fas fa-heart';
...
function openGallery(gallery) {
    modalTitle.textContent = gallery.title || 'Sin título';
    modalBody.innerHTML = '';
